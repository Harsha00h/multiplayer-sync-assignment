...
import type { CounterState, PeerFrame, Room, RoomStats } from '../net/room.js';
import type { NetworkEmulation } from '../net/connection.js';
import { Renderer } from '../render.js';
import { formatMet, statusMatrix } from './console.js';
import { TopRail } from './TopRail.js';
import { Roster } from './Roster.js';
import { SwitchBank } from './SwitchBank.js';
...
export function RoomConsole({ roomId, clientId, name, onLeave }: RoomConsoleProps): ReactElement {
  ...
  const identity = useMemo(() => ({ clientId, name, roomId }), [clientId, name, roomId]);

  const [self, setSelf] = useState<PeerInfo | null>(null);
  const [peers, setPeers] = useState<PeerFrame[]>([]);
  const [stats, setStats] = useState<RoomStats | null>(null);
  const [target, setTarget] = useState<TargetState | null>(null);
  const [counter, setCounter] = useState<CounterState>({ value: 0, by: null });
  ...
  useEffect(() => {
    ...
    const offCounter = room.onCounterChange(setCounter);
  ...
  const tally = useCallback((delta: 1 | -1) => {
    roomRef.current?.sendAction({ type: 'counter', delta });
  }, []);

  // Whose callsign to print under the tally. Looks up the roster, falling back to "you".
  const counterBy = useMemo(() => {
    if (counter.by === null) return null;
    if (self && counter.by === self.pid) return self.name;
    return roomRef.current?.getPeers().find((p) => p.pid === counter.by)?.name ?? null;
  }, [counter.by, self]);
  ...
  return (
    <div className="console">
      <TopRail roomId={identity.roomId} met={met} matrix={matrix} onLeave={onLeave} />
